...
if __name__ == "__main__":
    ...
    for map in maps:
        ...
        for file_name in files:
            ...
            # Compile coverage and freq data per conditions as we go
            h_mapped = open("%s/mapping_summary_%s.csv" % (output_folder, "-".join([str(id) for id in file_ids])), mode="r")
            lines = h_mapped.readlines()
            h_mapped.close()
            h_mapped = open("%s/mapping_summary_%s.csv" % (output_folder, "-".join([str(id) for id in file_ids])), mode="w")
            h_mapped.write(lines[0])
            total_reads = 0
            for ref in ref_names:
                total_reads += len(hits[ref][map])
            i = -1
            for l in lines[1:]:
                i+=1
                l = l.strip()
                h_mapped.write("%s,%s,%s\n" % (l,
                                               str(len(hits[ref_names[i]][map])),
                                               str(len(hits[ref_names[i]][map])*(10**6)/total_reads)))
            h_mapped.close()

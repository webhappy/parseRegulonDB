...
def printUnpairedStatus():
    print """
        ...
        """ % (total_count, badq / float(total_count) * 100, no_barcode_count / float(total_count) * 100, barcode_mismatch_count / float(total_count) * 100,100*Jordan_barcode_count/float(total_count)
        , " | ".join(["%s: %.2f" % (key, readperbc[key] / float(total_count) * 100) for key in readperbc if readperbc[key] != 0]),
...
if __name__ == "__main__":
    ...
    for id in file_ids:
        ...
        for record_R1 in records_R1:
            total_count += 1
            if verbose and total_count % 50000 == 0:
                printUnpairedStatus()
            ...
            if 'CGCCCTGCAGGGATGTCCACGAGG' in record_R1.seq:
                Jordan_barcode_count += 1
            else:
        ...
        """ % (total_count, no_barcode_count, barcode_mismatch_count, Jordan_barcode_count, " | ".join(["%s: %.2f" % (key, readperbc[key]/float(total_count)) for key in readperbc if readperbc[key] != 0 ]))
